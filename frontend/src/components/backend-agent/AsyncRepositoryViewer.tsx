...
interface AsyncRepositoryViewerProps {
  code: string;
}

export function AsyncRepositoryViewer({ code }: AsyncRepositoryViewerProps) {
    ...
    <div className="glass-panel p-6 rounded-2xl border border-white/10 space-y-4 font-mono text-xs">
      <div className="flex items-center justify-between border-b border-white/10 pb-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-bold text-white uppercase text-xs">Async SQLAlchemy 2.0 Repository Pattern (app/repositories/)</h3>
        </div>
        <button
          onClick={handleCopy}
